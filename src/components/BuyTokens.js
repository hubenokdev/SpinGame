...
const Buy = async(event, tokenAmount, change, price, account, admcAllowancement, dispatch) => {
  event.preventDefault();
  console.log("xxxxxxxxxxxx: ", admcAllowancement, " : ", tokenAmount, " : ", price, ":", account);

  if (admcAllowancement < tokenAmount * price) {
    console.log("xxxxxxxxxxxx2");
    try {
      await tokenService.Approve(tokenAmount * price);
...
const BuyTokens = ({account, price, admcAllowancement}) => {
  ...
  return (
    <Grid container rowSpacing={2}>
      ...
      <Grid item xs={12}>
        <form onSubmit={(event)=>Buy(event, tokenAmount.value, tokenAmount.change, price, account, admcAllowancement, dispatch)}>
          <Grid container spacing={2}>
            ...
            <Grid container alignItems="center" justifyContent="center" spacing={2}>
              <Button type="submit" size="large" variant="contained" color="warning">
                {admcAllowancement >= tokenAmount.value * price ? "Buy" : "Approve"}
              </Button>
            </Grid>
          </Grid>
          </Grid>
